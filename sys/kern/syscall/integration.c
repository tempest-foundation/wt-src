...
void
	...
	logger.printf("syscall", "info", "Initializing infrastructure\n");
	...
	if (!syscall_validate_infrastructure()) {
		logger.printf("syscall", "error", "Infrastructure validation failed!\n");
	...
	logger.printf("syscall", "info", "Infrastructure initialization complete\n");
...
void
	...
	logger.printf("syscall", "info", "Shutting down syscall infrastructure\n");
	...
	logger.printf("syscall", "success", "Infrastructure shutdown complete\n");
}
...
void
	...
	logger.printf(
	    "syscall", KNULL, "=====================================================\n");
	logger.printf(
	    "syscall", KNULL, "No.  | Name                    | Args | Calls\n");
	logger.printf(
	    "syscall", KNULL, "-----|-------------------------|------|----------\n");
...
kuint8_t
	...
	logger.printf("syscall", "info", "Validating infrastructure integrity...\n");
	...
	if (!syscalls.is_valid(SYS_EXIT)) {
		logger.printf("syscall", "error", "SYS_EXIT not registered\n");
		validation_passed = 0;
	}

	if (!syscalls.is_valid(SYS_GETPID)) {
		logger.printf("syscall", "error", "SYS_GETPID not registered\n");
		validation_passed = 0;
	}
	...
	if (syscalls.is_valid(999)) {
		logger.printf("syscall", "error", "Invalid syscall validation failed\n");
		validation_passed = 0;
	}

	if (validation_passed) {
		logger.printf("syscall", "success", "Infrastructure validation PASSED\n");
	} else {
		logger.printf("syscall", "error", "Infrastructure validation FAILED\n");
	}
...
void
	...
	if (max_calls > 0) {
		...
		logger.printf("syscall", "success", "Rate limiting disabled\n");
	}
}
...
void
	...
	if (!regs) {
		logger.printf("syscall", "error", "NULL registers pointer\n");
		return;
	}

	logger.printf("syscall", KNULL, "info", "Register dump:\n");
	logger.printf(
...
void
	...
	if (syscall_status != SYSCALL_STATUS_READY) {
		logger.printf("syscall", "error", "Syscall infrastructure not ready\n");
		return;
	}
	...
	logger.printf("syscall", "notice", "getpid test returned: %llu\n", mock_regs.rax);
...
void
	...
	logger.printf("syscall", KNULL, "Benchmark results:\n");
	logger.printf("syscall", "info", "Iterations requested: %u\n", iterations);
	logger.printf("syscall", "info", "Actual calls made: %llu\n", calls_made);
	logger.printf("syscall", "success", "Benchmark complete\n");
}
